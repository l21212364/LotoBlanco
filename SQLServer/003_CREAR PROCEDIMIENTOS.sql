...
as
begin
 select IdTienda,Nombre,RFC,Direccion,Telefono,Activo from TIENDA
end
...
as
begin
 select IdProveedor,RFC,RazonSocial,Telefono,Correo,Direccion,Activo from PROVEEDOR
end
...
begin
	SET @Resultado = 1
	IF NOT EXISTS (SELECT * FROM PROVEEDOR WHERE RFC = @Rfc and IdProveedor != @IdProveedor)
...
begin
 ...
 t.IdTienda,t.RFC,t.Nombre[NombreTienda] ,t.Direccion[DireccionTienda],pt.PrecioUnidadCompra,pt.PrecioUnidadVenta,pt.Stock,pt.Iniciado
...
--PROCEDMIENTO PARA REPORTE TIENDA PRODUCTO
 ...
 begin
 select 
 t.RFC[Rfc Tienda],t.Nombre[Nombre Tienda],t.Direccion[Direccion Tienda],
...
begin
 ...
 t.Nombre[Nombre Tienda], t.RFC[Rfc Tienda],
